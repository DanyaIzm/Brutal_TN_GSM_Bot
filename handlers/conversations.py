...
# Добавить ГСМ
def add_gsm_start(update, context):
    if not is_admin(update, context):
        return ConversationHandler.END

    USER_INPUT.update({update.effective_chat.id: NEW_GSM_INFO})

    context.bot.send_message(chat_id=update.effective_chat.id, text="Введите дату ГСМ в формате DD.MM.YY")
    context.bot.send_message(chat_id=update.effective_chat.id,
                             text="Для отмены введите /cancel на любом пункте диалога")
    return 'init_gsm_date'


def add_gsm_date(update, context):
    try:
        USER_INPUT[update.effective_chat.id]['gsm_date'] = update.message.text
        date = update.message.text.split('.')
...
def add_gsm_shift(update, context):
    try:
        USER_INPUT[update.effective_chat.id]['shift'] = update.message.text
...
def add_gsm_driver_ln(update, context):
    try:
        USER_INPUT[update.effective_chat.id]['driver_ln'] = update.message.text
...
def add_gsm_car_number(update, context):
    try:
        USER_INPUT[update.effective_chat.id]['car_number'] = update.message.text
    ...
    context.bot.send_message(chat_id=update.effective_chat.id, text="Теперь введите количество ГСМ (л)")
    return 'init_gsm'


def add_gsm_gsm(update, context):
    try:
        USER_INPUT[update.effective_chat.id]['gsm'] = update.message.text
        int(update.message.text)
        ...
        context.bot.send_message(chat_id=update.effective_chat.id, text="Произошла ошибка.\n"
                                                                        "Данные введены некорректно.\n\n"
                                                                        "Введите колчество ГСМ (л) ещё раз")
        return 'init_gsm'
...
def add_gsm_cost(update, context):
    try:
        USER_INPUT[update.effective_chat.id]['cost'] = update.message.text
        int(update.message.text)
...
def add_gsm_info(update, context):
    try:
        USER_INPUT[update.effective_chat.id]['info'] = update.message.text
    except Exception as e:
        print(e)
    ...
    try:
        inserter = Inserter(db)
        inserter.insert_gsm(USER_INPUT[update.effective_chat.id]['gsm_date'],
                            USER_INPUT[update.effective_chat.id]['shift'],
                            USER_INPUT[update.effective_chat.id]['driver_ln'],
                            USER_INPUT[update.effective_chat.id]['car_number'],
                            int(USER_INPUT[update.effective_chat.id]['gsm']),
                            int(USER_INPUT[update.effective_chat.id]['cost']),
                            USER_INPUT[update.effective_chat.id]['info'])
        del USER_INPUT[update.effective_chat.id]
    except Exception as e:
